...
io.on('connection', (socket) => {
    ...
    socket.on('send-message', async (data) => {
        const username = activeSockets[socket.id];
        if (!username) return socket.emit('error', 'Не авторизован');

        const user = await User.findOne({ username });
        if (!user) return;
        if (user.isBanned) return socket.emit('error', 'Вы забанены');
    ...
    socket.on('delete-message', async (id) => {
        try {
            ...
            if(!msg) return socket.emit('error', 'Сообщение не найдено');

            const user = await User.findOne({ username });
            if (msg.username === username || user?.isAdmin) {
                await Message.findByIdAndDelete(id);
                io.to(msg.channelId).emit('message-deleted', id);
            }
        } catch(e) {
    ...
    socket.on('edit-message', async (data) => {
        ...
        if(msg && msg.username === username) {
            ...
            msg.isEdited = true;
            await msg.save();
            io.to(msg.channelId).emit('message-updated', { id: msg._id.toString(), newText: msg.text });
        }
    });
...
async function handleCommand(socket, user, text) {
    ...
    const channelId = Array.from(socket.rooms).find(r => r !== socket.id) || 'global';
    
    if (cmd === '/ban' && target) {
        await User.findOneAndUpdate({ username: target }, { isBanned: true });
        const sysMsg = new Message({ channelId, username: 'SYSTEM', type: 'system', text: `⛔ ${target} забанен.` });
        await sysMsg.save();
        io.to(channelId).emit('message', formatMsg(sysMsg));
    }
    if (cmd === '/unban' && target) {
        await User.findOneAndUpdate({ username: target }, { isBanned: false });
        const sysMsg = new Message({ channelId, username: 'SYSTEM', type: 'system', text: `✅ ${target} разбанен.` });
        await sysMsg.save();
        io.to(channelId).emit('message', formatMsg(sysMsg));
    }
}
